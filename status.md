...
* **Deployed to production** (2026-07-26 ~20:40 UTC): fresh backup first (`gym_db_20260726_123733.sql.gz` + MinIO archive), both images built clean via `docker compose -p gym -f docker-compose.prod.yml build api web`, `up -d --no-deps api web` — the API's `prisma migrate deploy && node dist/main` boot command applied `20260726084628_add_announcements_closed_dates_device_tokens` automatically (additive only, no drops). **Verified live**: boot log shows the migration applying cleanly and all new routes mapped (`AnnouncementsModule`/`ClosedDatesModule` dependencies initialized, no errors); `\d` confirms `Announcement`/`ClosedDate`/`MemberDeviceToken` tables exist in the real production database; `GET https://gym.sparkco.vip/api` → 200; `GET /api/announcements`, `/api/closed-dates`, `/api/me/announcements` → 401 (routes live, auth required, not 404); `GET /app/announcements`, `/app/closed-dates` → 307 to sign-in (expected, unauthenticated). Both containers healthy.

Next

* Phase 1 (Android): no backend blockers remain. Phase 2 backend is now also unblocked and live — an Android developer can build Announcements/Closed Dates/push-token-registration against the real production contract today.
...
* No real owner sign-in or live announcement/closed-date creation was performed against production beyond the health/route checks above, deliberately, to avoid writing test data into the real tenant.
